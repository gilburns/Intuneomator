//
//  EntraGraphRequests+UploadDMG.swift
//  Intuneomator
//
...
// MARK: - DMG Upload Extension

/// Extension for handling macOS DMG application uploads to Microsoft Intune
/// Provides complete workflow for uploading DMG files with metadata, encryption, and group assignments
extension EntraGraphRequests {
    
    // MARK: - DMG Application Upload
    // Microsoft Graph API Reference: https://learn.microsoft.com/en-us/graph/api/intune-apps-macosdmgapp-create?view=graph-rest-beta
    
    /// Uploads a macOS DMG application to Microsoft Intune with complete metadata and file processing
    /// Handles the full upload workflow including encryption, chunked upload, and post-upload configuration
    /// - Parameters:
    ...
    static func uploadDMGApp(authToken: String, app: ProcessedAppResults, operationId: String? = nil, existingAppId: String? = nil) async throws -> String {
            ...
            if let httpResponse = metadataResponse as? HTTPURLResponse {
                ...
                if !(200...299).contains(httpResponse.statusCode) {
                    ...
                    throw NSError(domain: "UploadDMGApp", code: httpResponse.statusCode, userInfo: [
                        NSLocalizedDescriptionKey: "Failed to create DMG app metadata. Status: \(httpResponse.statusCode). \(responseBody)",
        ...
        do {
            // Create content version for the DMG file
            ...
            // Step 3: Encrypt DMG file for secure upload
