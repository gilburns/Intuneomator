//
//  EntraGraphRequests+UploadPKG.swift
//  Intuneomator
//
...
// MARK: - PKG Upload Extension

/// Extension for handling macOS PKG application uploads to Microsoft Intune
/// Provides complete workflow for uploading PKG files with pre/post-install scripts, metadata, and assignments
extension EntraGraphRequests {
    
    // MARK: - PKG Application Upload
    // Microsoft Graph API Reference: https://learn.microsoft.com/en-us/graph/api/intune-apps-macospkgapp-create?view=graph-rest-beta
    
    /// Uploads a macOS PKG application to Microsoft Intune with script support and complete configuration
    /// Handles PKG files with optional pre-install and post-install scripts for advanced deployment scenarios
    /// - Parameters:
    ...
    static func uploadPKGWithScripts(authToken: String, app: ProcessedAppResults, operationId: String? = nil) async throws -> String {
        ...
        do {
            // Create content version for the PKG file
            ...
            // Step 3: Encrypt PKG file for secure upload
