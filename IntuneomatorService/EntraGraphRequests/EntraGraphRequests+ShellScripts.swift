...
extension EntraGraphRequests {
    ...
    ///
    /// Fetches detailed execution results for a  shell script across all assigned devices,
    ...
    /// **Usage Example:**
    /// ```swift
    /// let deviceStates = try await EntraGraphRequests.getShellScriptDeviceRunStates(
    ///     authToken: token,
    ///     scriptId: "script-guid"
    /// )
    ///
    ...
    /// - Audit trail for shell script runs
    ///
    ...
    /// - Throws:
    ///   - `NSError` with domain "ShellScriptDeviceStates" and code 500: Invalid URL, HTTP request failure, or JSON parsing failure
    ///   - Network-related errors from URLSession
    ///
    /// **Required Permissions:**
    /// - DeviceManagementConfiguration.Read.All (Application or Delegated)
    ...
    ///
    /// **Microsoft Graph API:**
    /// - Endpoint: `GET /deviceManagement/deviceShellScripts/{id}/deviceRunStates?$expand=managedDevice`
    /// - Documentation: Microsoft Graph deviceManagementScriptDeviceState resource type
    static func getShellScriptDeviceRunStates(authToken: String, scriptId: String) async throws -> [[String: Any]] {
        
        Logger.info("Fetching device run states for shell script: \(scriptId)", category: .core)
        
        var allDeviceStates: [[String: Any]] = []
        var nextPageUrl: String? = "https://graph.microsoft.com/beta/deviceManagement/deviceShellScripts/\(scriptId)/deviceRunStates?$expand=managedDevice"
        var pageCount = 0
        
        // Follow pagination until all device run states are fetched
        while let urlString = nextPageUrl {
            guard let url = URL(string: urlString) else {
                throw NSError(domain: "ShellScriptDeviceStates", code: 500, userInfo: [NSLocalizedDescriptionKey: "Invalid pagination URL: \(urlString)"])
            }
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                throw NSError(domain: "ShellScriptDeviceStates", code: 500, userInfo: [NSLocalizedDescriptionKey: "Invalid response type"])
            }
            
            guard httpResponse.statusCode == 200 else {
                let responseStr = String(data: data, encoding: .utf8) ?? "Unknown error"
                Logger.error("Failed to fetch device run states for shell script \(scriptId) (page \(pageCount + 1)): \(responseStr)", category: .core)
                throw NSError(domain: "ShellScriptDeviceStates", code: 500, userInfo: [NSLocalizedDescriptionKey: "Failed to fetch device run states (page \(pageCount + 1)). Status: \(httpResponse.statusCode), Response: \(responseStr)"])
            }
            
            // Parse JSON response
            guard let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any] else {
                throw NSError(domain: "ShellScriptDeviceStates", code: 500, userInfo: [NSLocalizedDescriptionKey: "Invalid JSON response format for device run states (page \(pageCount + 1))"])
            }
            
            // Extract device run states from this page
            guard let pageDeviceStates = json["value"] as? [[String: Any]] else {
                throw NSError(domain: "ShellScriptDeviceStates", code: 500, userInfo: [NSLocalizedDescriptionKey: "Invalid response format for device run states (page \(pageCount + 1))"])
            }
        ...
        Logger.info("Successfully fetched \(allDeviceStates.count) device run states for shell script \(scriptId) across \(pageCount) pages", category: .core)
